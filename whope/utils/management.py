...
async def get_risk_from_message(prediction: List[float], emotion_mapping: Dict[int, str], last_message: Dict[str, str], old_risk: float) -> float:
    ...
    return tanh(risk)  # risk must be kept between 0 and 1
...
async def get_enriched_message(messages: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    old_risk: float = past_messages[-1]["risk"] if past_messages else 0.0
    ...
    for message in past_messages:
        message_past_questions: List[str] = message.get("past_questions", [])
        if message_past_questions:
            past_questions.append(message_past_questions[-1])
