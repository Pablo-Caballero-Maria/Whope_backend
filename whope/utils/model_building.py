...
def build_model_dense(hp: kt.HyperParameters, bert_model: TFBertModel) -> Model:
    ...
    model.compile(
        optimizer=Adam(learning_rate=hp.Float('learning_rate', min_value=MIN_LEARNING_RATE, max_value=MAX_LEARNING_RATE, sampling='LOG')),
...
def get_models_and_histories(model_types: List[str]) -> List[Tuple[Model, History]]:
    ...
    input_ids_train, input_ids_val, y_train, y_val = train_test_split(
        encoded_inputs_np["input_ids"], labels, test_size=TEST_SIZE, random_state=RANDOM_STATE
    )

    attention_mask_train, attention_mask_val = train_test_split(
        encoded_inputs_np["attention_mask"], test_size=TEST_SIZE, random_state=RANDOM_STATE
    )
