...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code: int) -> None:
        ...
        await self.channel_layer.group_discard(
            self.room_name, self.user.get("channel_name", None)
        )
        ...
        await self.channel_layer.group_send(
            self.room_name, {"type": "user_disconnection"}
        )
        ...
        if "virtual_room" in self.room_name:
            ...
            await self.send(
                text_data=json.dumps(
                    {"message": f"Your message {message} has been sent."}
                )
            )
        else:
            await self.channel_layer.group_send(
    ...
    async def assign_room(self) -> None:
        if self.user.get("is_worker", False) == "True":
            ...
            # to other events. thus the necessity of monitoring it asynchronously by using asyncio.create_task(monitor_celery_task)
            ...
            celery_task: AsyncResult = await sync_to_async(
                check_for_non_workers_task.delay
            )(self.user.get("username", None))
    ...
    async def process_non_worker_assignment(self, worker_username, free_non_worker):
        ...
        new_room_name: str = (
            f"{worker_username}_{free_non_worker['username']}_real_room"
        )
        ...
        await self.channel_layer.group_add(
            new_room_name, self.user.get("channel_name", None)
        )
        await self.channel_layer.group_add(
            new_room_name, free_non_worker.get("channel_name", None)
        )

    async def set_user_status(self, user: Dict[str, str], status: str) -> None:
        users: AsyncIOMotorCollection = await self.get_users()
        await users.update_one(
            {"_id": ObjectId(user.get("user_id", None))}, {"$set": {"status": status}}
        )
    ...
    async def preprocess_message(self, message: str, user: Dict[str, str]):
        (
            await self.send(
                text_data=json.dumps({"error": "Message must not be empty."})
            )
            if not message
            else None
        )
    ...
    async def listen_to_rabbit(self) -> None:
        ...
        await channel.set_qos(
            prefetch_count=1
        )  # Controla la cantidad de mensajes sin procesar permitidos
        ...
        async def on_message(message: IncomingMessage) -> None:
            async with message.process():
                ...
                if worker_username == self.user.get("username"):
                    await self.process_non_worker_assignment(
                        worker_username, free_non_worker
                    )
