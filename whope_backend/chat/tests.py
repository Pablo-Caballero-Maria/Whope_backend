...
class TestChatConsumer(TransactionTestCase):
    ...
    async def websocket_connect(self, room_name: str, token: str = "") -> WebsocketCommunicator:
    ...
    async def test_send_and_receive_message(self) -> None:
        ...
        communicator: WebsocketCommunicator = await self.websocket_connect("testroom", token)
    ...
    async def test_invalid_token(self) -> None:
        communicator: WebsocketCommunicator = await self.websocket_connect("testroom", "invalid_token")
        message_data: Dict[str, str] = {
            'message': 'Hello, world!',
        }
        await communicator.send_json_to(message_data)
